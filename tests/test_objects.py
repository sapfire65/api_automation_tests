...
class TestObjects:
    ...
    @allure.title("Получаем объекты из базы")
    ...
    def test_get_objects(self, client, request):
        ...
        print(f'{S_TEXT}ответ: {response}{E_Text}')

        """убеждаемся, что в ответ пришли объекты, которые мы ожидаем"""
        assert_status_code(response, HTTPStatus.OK) # сравнивает код ответа от сервера с ожидаемым
        assert_response_body_fields(request, response) # ответ от сервера, сравнивает ожидаемый объект с полученным


    @allure.title("Получение заранее заготовленных объектов из базы с параметром ids")
    @allure.severity("Critical")
